...
// BenchmarkR1004CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR1004CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("exec-from-mount.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create an exec event for execution from mount
	e := &events.ExecEvent{
		Event: tracerexectype.Event{
			Event: eventtypes.Event{
				CommonData: eventtypes.CommonData{
