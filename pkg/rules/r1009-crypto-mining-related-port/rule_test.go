...
// BenchmarkR1009CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR1009CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("crypto-mining-related-port.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create a network event for crypto mining port
	e := &tracernetworktype.Event{
		Event: eventtypes.Event{
			CommonData: eventtypes.CommonData{
				K8s: eventtypes.K8sMetadata{
	...
	for i := 0; i < b.N; i++ {
		...
		_, err := celEngine.EvaluateRule(eventMap, utils.NetworkEventType, ruleSpec.Rules[0].Expressions.RuleExpression)
		if err != nil {
