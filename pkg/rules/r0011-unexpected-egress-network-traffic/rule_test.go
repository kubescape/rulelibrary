...
// BenchmarkR0011CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR0011CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("unexpected-egress-network-traffic.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create a network event for egress traffic
	e := &tracernetworktype.Event{
		Event: eventtypes.Event{
			CommonData: eventtypes.CommonData{
				K8s: eventtypes.K8sMetadata{
