...
// BenchmarkR1011CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR1011CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("ld-preload-hook.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create an open event for ld.so.preload
	e := &events.OpenEvent{
		Event: traceropentype.Event{
			Event: eventtypes.Event{
				CommonData: eventtypes.CommonData{
