...
// BenchmarkR1000CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR1000CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("exec-from-malicious-source.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create an exec event for malicious source
	e := &events.ExecEvent{
		Event: tracerexectype.Event{
			Event: eventtypes.Event{
				CommonData: eventtypes.CommonData{
			...
			Pid:     1234,
			Comm:    "malicious",
			ExePath: "/dev/shm/malicious",
			Cwd:     "/",
			Args:    []string{"/dev/shm/malicious"},
		},
	}
