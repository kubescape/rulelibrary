...
// BenchmarkR0006CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR0006CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("unexpected-service-account-token-access.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create an open event for service account token access
	e := &traceropentype.Event{
		...
		FullPath: "/var/run/secrets/kubernetes.io/serviceaccount/token",
