...
// BenchmarkR1010CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR1010CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("symlink-created-over-sensitive-file.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create a symlink event for sensitive file
	e := &tracersymlinktype.Event{
		Event: eventtypes.Event{
			CommonData: eventtypes.CommonData{
				K8s: eventtypes.K8sMetadata{
	...
	for i := 0; i < b.N; i++ {
		...
		_, err := celEngine.EvaluateRule(eventMap, utils.SymlinkEventType, ruleSpec.Rules[0].Expressions.RuleExpression)
		if err != nil {
