...
// BenchmarkR0007CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR0007CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("kubernetes-client-executed.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create an exec event for kubectl
	e := &events.ExecEvent{
		Event: tracerexectype.Event{
			Event: eventtypes.Event{
				CommonData: eventtypes.CommonData{
