...
// BenchmarkR1030CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR1030CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("unexpected-io_uring-operation.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create an io_uring event for unexpected operation
	e := &traceriouringtype.Event{
		Event: eventtypes.Event{
			CommonData: eventtypes.CommonData{
				K8s: eventtypes.K8sMetadata{
	...
	for i := 0; i < b.N; i++ {
		...
		_, err := celEngine.EvaluateRule(eventMap, utils.IoUringEventType, ruleSpec.Rules[0].Expressions.RuleExpression)
		if err != nil {
