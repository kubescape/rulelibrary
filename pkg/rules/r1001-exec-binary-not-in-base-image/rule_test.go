...
// BenchmarkR1001CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR1001CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("exec-binary-not-in-base-image.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create an exec event for binary not in base image
	e := &events.ExecEvent{
		Event: tracerexectype.Event{
			Event: eventtypes.Event{
				CommonData: eventtypes.CommonData{
