...
func TestR0001UnexpectedProcessLaunched(t *testing.T) {
	...
	// Evaluate the rule
	ok, err := celEngine.EvaluateRule(enrichedEvent, utils.ExecveEventType, ruleSpec.Rules[0].Expressions.RuleExpression)
	if err != nil {
...
func BenchmarkEvaluateRuleNative(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, _ = celEngine.EvaluateRule(enrichedEvent, utils.ExecveEventType, ruleSpec.Rules[0].Expressions.RuleExpression)
	}
}
