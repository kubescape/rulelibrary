...
// BenchmarkR0008CELEvaluation benchmarks the CEL rule evaluation performance
func BenchmarkR0008CELEvaluation(b *testing.B) {
	ruleSpec, err := common.LoadRuleFromYAML("read-environment-variables-procfs.yaml")
	if err != nil {
		b.Fatalf("Failed to load rule: %v", err)
	}

	// Create an open event for proc environ access
